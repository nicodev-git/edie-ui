...
const pickZone = (opts, frameBounds, targetBounds, size) => {
  ...
  */
  ...
  if (opts.place) {
    ...
    /* If there is a preferPlace, we will try to use it if possible */
    if (opts.preferPlace) {
      const preferType = getPreferenceType(opts.preferPlace)
      if (!preferType) throw createPreferenceError(opts.preferPlace)

      const preferAvailZone = availZones.filter((zone) => (
        /* Here we check both type & preferType to avoid conflict setup
        For example:
        - { place: "column", preferPlace: "left" }
        - { place: "column", preferPlace: "row" } */
        zone[type] === opts.place &&
        zone[preferType] === opts.preferPlace
      ))[0]

      if (preferAvailZone) return preferAvailZone
    }
