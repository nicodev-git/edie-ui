...
class ThreatMap extends React.Component {
            ...
            <div style={{flex: 1}} className="flex-vertical">
                <div className="form-inline padding-sm">
                    ...
                    <div className={"form-group " + (this.state.mode == 'real' ? '' : 'hidden')}>
                        ...
                        <div className={"inline margin-md-left " + (this.state.history ? "" : "hidden")}>
                            ...
                            <DatePicker ref="dateFrom" readOnly={true}
                    ...
                    <div className="form-group pull-right inline">
                        ...
                        <a href="javascript:;" onClick={this.onClickSettings.bind(this)}>
                            <i className="fa fa-x fa-cog valign-middle"></i>
                        </a>
                    </div>
                ...
                <div style={{flex: 1, position: 'relative'}} className={(this.state.history || this.state.mode == 'demo') ? 'slider-visible' : ''}>
                    ...
                    <div className="play-controls">
                        ...
                        <div className="incident-time hidden" style={{color:"white", paddingTop: "3px"}}></div>
        ...
        return (
            <div id="countryDrillDownContainer"
                ...
                <div id="countryDrillDowntitleContainer">
                    <div style={{marginRight:"5px"}}></div>Info
                </div>
    ...
    onViewportChange() {
        ...
        var scenes = me.currentPlay.scene;
        ...
        me.blasts.forEach(item => {
            ...
            if (trans < 2) {
                ...
                obj.transition()
                    ...
                    .each('end', function(d){
                        var callback = d.callback;
                        var index = me.connections.indexOf(item);
                        if (index >= 0) me.connections.splice(index, 1);
                        if(callback) callback();
                    })
            }
    ...
    updateObjectPosition(device) {
        ...
        var img = device.imgObj;
        if (img) {
            //device.country
            var latlng = me.getCountryLatLng(device.country);
            if (!latlng) return;
            var xy = me.latLngToXY(latlng[0], latlng[1]);
            device.x = xy[0];
            device.y = xy[1];
    ...
    onTickReal() {
        ...
        $.get(Api.bi.threatMapEventsReal + '?minutes=1', {
            ...
            var scenes = me.buildScene(incidents);
    ...
    onSeek(newpos) {
        ...
        var scene = me.currentPlay.scene;
    ...
    addAttackRow(id, attacker, target, time, type, action, severity) {
        ...
        if(me.buffer.length > 10){
            var removed = me.buffer.splice(5);
            // $.each(removed, function(i, item){
            //     item.find('.sourceCol p').tooltip('destroy');
            //     item.find('.destCol p').tooltip('destroy');
            // });
    ...
    showBlast(device, color, callback) {
        ...
        img.attr('x', device.x - image.width / 2)
            ...
            .each('end', (d) => {
                var callback = d.callback;
                var index = me.blasts.indexOf(img);
    ...
    getHistoryEvents(cb) {
        ...
        $.get(Api.bi.threatMapEventsHistory, {
            ...
            if (res.success && cb) {
                var scene = me.buildScene(res.object);
                me.currentPlay.scene = scene;
                cb(scene);
            }
        }).always(() => {
    ...
    buildScene(incidents) {
        ...
        incidents.forEach(item => {
            var deviceIP = item['destinationip'];
            var device = devices[deviceIP];
            ...
            var attackerIP = item['ipcountry'];
            var attacker = attackers[attackerIP];
            if (!attacker) {
                attacker = {
                    id: attackerIP,
            ...
            var severity = item['incidentseverity'].toLowerCase();
    ...
    latLngToXY (lat, lng){
        var me = this;
        var pos = me.vmap.latLngToPoint(lat, lng);
        return [pos.x, pos.y];
    }
    ...
    getCountryLatLng (name) {
        ...
        var code = found[0].alpha2
        if(!code) return null

        var latlng = country_latlng[code];
