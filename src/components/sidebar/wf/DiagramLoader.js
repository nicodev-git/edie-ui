...
function addLine2(data, startObj, endObj, startPoint, endPoint, items) {
  ...
  const startPos = leftTpl && leftTpl.getConnectionPoint ? leftTpl.getConnectionPoint(startObj, startPoint) : {
    x: 0,
    y: 0
  }
  const endPos = rightTpl && rightTpl.getConnectionPoint ? rightTpl.getConnectionPoint(endObj, endPoint) : {x: 0, y: 0}

  const stepPoints = leftTpl && leftTpl.getConnectionPoint ? findStepPoints(leftTpl, startPos, startPoint, rightTpl, endPos, endPoint) : [{
    x: 0,
    y: 0
  }]
  const handlePoints = rightTpl && rightTpl.getConnectionPoint ? getHandlePoints(startPos, stepPoints, endPos) : [{
    x: 0,
    y: 0
  }]
